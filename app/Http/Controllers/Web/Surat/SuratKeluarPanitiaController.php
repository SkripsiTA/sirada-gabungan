...
class SuratKeluarPanitiaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            'lepihan_surat' => 'required',
            'lepihan_surat.*' => 'mimes:jpeg,jpg,pdf|max:3072',
        ]);
        ...
        if($request->hasFile('lepihan_surat')) {
            ...
            foreach ($request->file('lepihan_surat') as $lampiran) {
                $filename = time().'.'.$lampiran->getClientOriginalExtension();
                $lampiran->move('assets/file/lampiran/surat-keluar', $filename);
                $data[] = $filename;
            }
        ...
        $lampiransurat = new Lampiran;
        $lampiransurat->file = json_encode($data);
        $lampiransurat->surat_keluar_id = $suratkeluar->surat_keluar_id;
        $lampiransurat->save();
